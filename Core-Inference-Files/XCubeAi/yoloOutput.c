...
int postprocess_single(float *data, int w, int h, float stride, int num_classes, const int anchors[3][2],
    ...
    if(stride == 16.)
    	...
    	a = 360;
    for (int i = 1; i < a+1; i++) {
  	  LC_PRINT("%.4f,", data[i-1]);
  	  if(i % 6 == 0){
  		LC_PRINT("\r\n");
  	  }

    }
    LC_PRINT("DEBUG FINISHED\n \n");
     */
    ...
    while (1) {
        ...
        for (int i = 0; i < candicate_num; i++) {
            if (!suppress[i]) {
                ...
                //LC_PRINT("Scorer %f\r\n", candidates[max_index].score);
                //LC_PRINT("Scorer %f\r\n", candidates[i].score);
                //LC_PRINT("Overunion %f\r\n", ovr);
                //LC_PRINT("\r\n");
            }
        }
    }
...
/*------ Set the number of inputs, for 3 output feature maps its input_data[3]-------------*/
/*------ Set the Anchor size, for 3 output feature maps its anchors[3][3][2]-------------*/
det_box **postprocessing(float *input_data[2],
                         int w, int h, int output_c, int num_classes, const int anchors[2][3][2], int outputs,
                         const float NMS_threshold, const float VALID_THRESHOLD, int *box_ret, det_box **ret_box) {

	/*------ Set the number of classes-------------*/
    det_box *candidates[1];
    int i;
    /*------ Allocates RAM for BB-------------*/
    for (i = 0; i < num_classes; i++) {
        candidates[i] = (det_box *)malloc(sizeof(det_box) * MAX_BOX_PER_CLASS);
    ...
    /*------ Allocates RAM for NMS filtered BB-------------*/
    for (i = 0; i < num_classes; i++) {
        ret_box[i] = (det_box *)malloc(sizeof(det_box) * MAX_BOX_PER_CLASS);
        if (ret_box[i] == NULL) {
            // Handle error
            return NULL;
        }
    ...
    /*------ Employed Strides, for YOLOv3 {8, 16, 32} -------------*/
    const int strides[] = {16, 32};

    /*------ For each Stride, Yolov3 its 3 -------------*/
    for (i = 0; i < 2; i++) {

    	/*------ Process each output layer -------------*/
        int size = (w / strides[i]) * (h / strides[i]) * output_c;
        postprocess_single(input_data[i], w / strides[i], h / strides[i], strides[i], num_classes, anchors[i], 3, candidates,
                           VALID_THRESHOLD);
    }
    LC_PRINT("NMS Start!\r\n");
    for (i = 0; i < num_classes; i++) {
